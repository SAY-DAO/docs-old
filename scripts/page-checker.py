...
htmlToTest = (
    markdownToTest.replace("docs/", "docs/.vuepress/dist/")
    .replace("_index.md", "index.html")
...
for link in links:
    ...
    if (
        "http" in linkUrl
        and "docs.filecoin.io" not in linkUrl
        and "https://github.com/SAY-DAO/road-map-docs" not in linkUrl
        and (linkUrl + "|") not in checkedLinks
    ):
        # print(linkUrl)
        ...
        try:
            ...
            if r.status_code != 200:
                outputText += (
                    "- The following link returned HTTP status code "
                    + str(r.status_code)
                    + " ("
